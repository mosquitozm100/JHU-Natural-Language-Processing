...
import random
def dfs(gram_list):         #gram_list is the gram you need to use in a list form, the initail gram_list shoule be ['ROOT']. iter_steps stands for the steps the program has already iterated 
    global M
    if M <= 0:
        return ['...']
    M = M - 1
    ...
    for index in range(1, len(gram_list)):
        ...
        #print('choice:', choice)
        new_list = dfs(non_term_dict[symbol][choice])
