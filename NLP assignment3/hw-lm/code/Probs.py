...
class LogLinearLanguageModel(LanguageModel):
    ...
    def prob(self, x: str, y: str, z: str) -> float:
        ...
        uxyz = math.exp(np.dot(np.dot(xv, self.X),zv.T)+np.dot(np.dot(yv,self.Y),zv.T))
        zxy = 0
        for name in self.vocab:
            name = self.replace_missing(name)
            item = np.array(self.vectors[name])
            zxy += math.exp(np.dot(np.dot(xv, self.X),item.T)+np.dot(np.dot(yv,self.Y),item.T))
        return uxyz/zxy
            ...
            for i in range(2, len(tokens_list)):
                ...
                for row in range(self.dim):
                    for col in range(self.dim):
                        gradient_X[row][col] = xv[row]*zv[col]
                        gradient_Y[row][col] = yv[row]*zv[col]
