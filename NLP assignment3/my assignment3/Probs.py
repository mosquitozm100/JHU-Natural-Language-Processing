...
class LanguageModel:
    ...
    def make(cls, smoother: str, lexicon: Path) -> "LanguageModel":
        ...
        if smoother_name == "uniform":
            ...
            #assert lambda_ is not None
            return LogLinearLanguageModel(1, lexicon)
        else:
...
class BackoffAddLambdaLanguageModel(LanguageModel):
    ...
    def prob(self, x: str, y: str, z: str) -> float:
        ...
        # over all values of typeZ will give 1, so sum_z p(z | ...) = 1
        # as is required for any probability function.
        return (self.tokens[x, y, z] + self.lambda_ * self.vocab_size * self.prob_yz(y,z)) / (
            self.tokens[x, y] + self.lambda_ * self.vocab_size
        )
    
    def prob_yz(self, y: str, z: str) -> float:
        assert self.vocab is not None
        y = self.replace_missing(y)
        z = self.replace_missing(z)
        # Notice that summing the numerator over all values of typeZ
        ...
        # over all values of typeZ will give 1, so sum_z p(z | ...) = 1
        # as is required for any probability function.
        return (self.tokens[y, z] + self.lambda_ * self.vocab_size * self.prob_z(z)) / (
            self.tokens[(y,)] + self.lambda_ * self.vocab_size
        )

    def prob_z(self, z: str) -> float:
        assert self.vocab is not None
        z = self.replace_missing(z)
        # Notice that summing the numerator over all values of typeZ
        # will give the denominator.  Therefore, summing up the quotient
        # over all values of typeZ will give 1, so sum_z p(z | ...) = 1
        # as is required for any probability function.
        return (self.tokens[(z,)] + self.lambda_) / (
            self.tokens[()] + self.lambda_ * self.vocab_size
        )
...
class LogLinearLanguageModel(LanguageModel):
    ...
    def prob(self, x: str, y: str, z: str) -> float:
        ...
        y = self.replace_missing(y)
        z = self.replace_missing(z)
        vec_x = np.array(self.vectors[x])
        vec_y = np.array(self.vectors[y])
        vec_z = np.array(self.vectors[z])
        #print(vec_x)
        #print(vec_y)
        #print(vec_z)
        uxyz = self.Uxyz(vec_x,vec_y,vec_z)
        return uxyz / self.Vxy(vec_x, vec_y)

    def Uxyz(self, vec_x, vec_y, vec_z) -> float:
        #print(vec_x, vec_y,vec_z)
        #print((vec_x.dot(np.array(self.X)).dot(vec_z)), (vec_y.dot(np.array(self.Y)).dot(vec_z)))
        return math.exp((vec_x.dot(np.array(self.X))).dot(vec_z)+ (vec_y.dot(np.array(self.Y))).dot(vec_z))

    def Vxy(self, vec_x, vec_y) -> float:
        Vxy_num = 0
        for tmp_vec_z in self.vocab:
            tmp_vec_z = self.replace_missing(tmp_vec_z)
            tmp_vec_z = np.array(self.vectors[tmp_vec_z])
            tmp_uxyz = self.Uxyz(vec_x,vec_y,tmp_vec_z)
            Vxy_num = Vxy_num + tmp_uxyz
        return Vxy_num
    ...
    def train(self, corpus: Path) -> List[str]:
        ...
        #####################
        theta = np.zeros(self.dim * self.dim + self.dim * self.dim)
        t = 0
        ...
        for e in range(epochs):
            ...
            for i in range(2, len(tokens_list)):
                ...
                sx, sy, sz = tokens_list[i - 2], tokens_list[i - 1], tokens_list[i]

                sx = self.replace_missing(sx)
                sy = self.replace_missing(sy)
                sz = self.replace_missing(sz)

                x = np.array(self.vectors[sx])
                y = np.array(self.vectors[sy])
                z = np.array(self.vectors[sz])

                vxy = self.Vxy(x,y)

                gradient_X = (x.reshape(x.shape[0],1)).dot(z.reshape(1, z.shape[0]))
                #print("gradient_X", gradient_X)
                gradient_Y = (y.reshape(y.shape[0],1)).dot(z.reshape(1, z.shape[0]))

                for z_prime in self.vocab:
                    #print("z_prime=", z_prime)
                    z_prime = self.replace_missing(z_prime)
                    #print("z_prime", z_prime)
                    z_prime = np.array(self.vectors[z_prime])
                    #print("gradient_X", gradient_X)
                    gradient_X -= self.Uxyz(x,y,z_prime) / vxy * (x.reshape(x.shape[0],1)).dot(z_prime.reshape(1, z_prime.shape[0]))
                    #print((x.reshape(x.shape[0],1)).dot(z_prime.reshape(1, z_prime.shape[0])))
                    gradient_Y -= self.Uxyz(x,y,z_prime) / vxy * (y.reshape(y.shape[0],1)).dot(z_prime.reshape(1, z_prime.shape[0]))

                gradient_X -= (2 * self.c / self.N) * np.array(self.X)
                gradient_Y -= (2 * self.c / self.N) * np.array(self.Y)
                
                self.X = (np.array(self.X) + gamma * gradient_X).tolist()
                self.Y = (np.array(self.Y) + gamma * gradient_Y).tolist()

                Fi = math.log(self.prob(sx,sy,sz))# - self.c / self.N * (np.linalg.norm(np.array(self.X)) ** 2 + np.linalg.norm(np.array(self.Y)) ** 2)
                #print("Fi:", Fi)
            ...
            #for i in range(2, len(tokens_list)):
                ...
                #F = F + Fi
            F -= self.c * (np.linalg.norm(np.array(self.X)) ** 2 + np.linalg.norm(np.array(self.Y)) ** 2)
            F = F / self.N
            print(f"epoch {e + 1}: F={F}")
        print(f"Finished training on {self.N} tokens")
